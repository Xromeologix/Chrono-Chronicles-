...
export function Effects() {
  ...
  const playerHookedBy = useGameStore(state => state.playerHookedBy);
  ...
  return (
    <>
      ...
      {playerHookedBy && (
        <HookLine start={camera.position.toArray() as [number, number, number]} end={playerHookedBy.position} />
      )}
    </>
  );
}

function HookLine({ start, end }: { start: [number, number, number], end: [number, number, number] }) {
  const ref = useRef<THREE.Mesh>(null);
  
  const { position, rotation, length } = useMemo(() => {
    const s = new THREE.Vector3(...start);
    const e = new THREE.Vector3(...end);
    // Lower the start position slightly so it doesn't block the camera entirely
    s.y -= 0.5;
