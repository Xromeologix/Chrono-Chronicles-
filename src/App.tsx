...
function HUD() {
  ...
  const isExecuted = useGameStore(state => state.isExecuted);
  ...
  return (
    <>
      {/* Health Bar */}
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-64 md:w-96 pointer-events-none z-40">
        <div className="text-cyan-400 font-mono text-sm mb-1 flex justify-between">
          <span>INTEGRITY</span>
          <span>{Math.ceil(playerHealth)}%</span>
        </div>
        <div className="h-4 bg-black/50 border border-cyan-400/30 rounded-full overflow-hidden">
          <div 
            className={`h-full transition-all duration-200 ${playerHealth > 50 ? 'bg-cyan-400' : playerHealth > 25 ? 'bg-yellow-400' : 'bg-red-500'}`}
      ...
      {playerState === 'disabled' && (
        <div className="absolute inset-0 bg-red-500/20 pointer-events-none flex items-center justify-center overflow-hidden">
          {isExecuted && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="absolute w-[200%] h-8 bg-red-600 rotate-45 origin-center animate-slash shadow-[0_0_30px_rgba(220,38,38,1)]" />
              <div className="absolute w-[200%] h-8 bg-red-600 -rotate-45 origin-center animate-slash-delayed shadow-[0_0_30px_rgba(220,38,38,1)] opacity-0" />
            </div>
          )}
          <div className="text-red-500 text-4xl md:text-6xl font-black tracking-widest drop-shadow-[0_0_20px_rgba(239,68,68,1)] animate-pulse text-center z-10">
            {isExecuted ? 'EXECUTED' : 'SYSTEM DISABLED'}
          </div>
        </div>
      )}
    ...
    <div className="w-screen h-screen bg-black relative overflow-hidden font-mono select-none">
      ...
      {gameState === 'menu' && (
        <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center z-10 pointer-events-auto">
          <h1 className="text-6xl font-black text-cyan-400 mb-8 drop-shadow-[0_0_20px_rgba(34,211,238,0.8)] tracking-tighter text-center">
            {gameMode === 'nexus' ? 'NEON ARENA' : 'CHRONOS CLASH'}
          </h1>
          <p className="text-gray-400 mb-8 text-center max-w-md">
            {isMobile ? 'Swipe left/right to look. Use joystick to move.' : 'WASD to move. Mouse to look and shoot.'}<br/>
            Hit enemies for points. Don't get hit!
          </p>

          <div className="flex flex-col gap-6 w-80">
            <div className="flex bg-black/50 border border-cyan-900/50 rounded p-1">
              <button
                onClick={() => setGameMode('nexus')}
                className={`flex-1 py-2 text-sm font-bold transition-all ${gameMode === 'nexus' ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-400' : 'text-gray-500 hover:text-cyan-400/50'}`}
              >
                THE NEXUS
              </button>
              <button
                onClick={() => setGameMode('chronos')}
                className={`flex-1 py-2 text-sm font-bold transition-all ${gameMode === 'chronos' ? 'bg-fuchsia-500/20 text-fuchsia-400 border border-fuchsia-400' : 'text-gray-500 hover:text-fuchsia-400/50'}`}
              >
                CHRONOS CLASH
              </button>
            </div>
            ...
            <button
              onClick={() => startGame()}
              className={`w-full px-8 py-4 border-2 text-xl font-bold rounded transition-all duration-200 ${gameMode === 'nexus' ? 'bg-cyan-500/20 border-cyan-400 text-cyan-400 hover:bg-cyan-400 hover:text-black shadow-[0_0_15px_rgba(34,211,238,0.5)]' : 'bg-fuchsia-500/20 border-fuchsia-400 text-fuchsia-400 hover:bg-fuchsia-400 hover:text-black shadow-[0_0_15px_rgba(232,121,249,0.5)]'}`}
            >
              PLAY NOW
            </button>
          </div>
